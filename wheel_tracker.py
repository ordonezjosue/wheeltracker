...
if tt_file is not None:
    try:
        ...
        df_tt = df_tt_raw[~df_tt_raw["Underlying Symbol"].astype(str).str.contains("/", na=False)].copy()
        df_tt["Strategy"] = "Imported"
        df_tt["Process"] = "Imported"
        df_tt.rename(columns={
            ...
            "Strike Price": "Strike",
            "Quantity": "Qty",
            "Price": "Credit Collected",
            "P/L Open": "P/L"
        }, inplace=True)
        df_tt["Result"] = df_tt["P/L"].apply(lambda x: "Closed" if pd.notnull(x) else "Open")
        df_tt["P/L"] = pd.to_numeric(df_tt["P/L"], errors="coerce").fillna(0)
    except Exception as e:
...
# 📊 Metrics Dashboard
# ============================
if not df_wheel.empty or not df_pcs.empty or not df_tt.empty:
    combined_df = pd.concat([df_wheel, df_pcs, df_tt], ignore_index=True)
...
if df_wheel.empty and df_pcs.empty and df_tt.empty:
